...
public class CategoriaController {
	...
	public ResponseEntity<Categoria> updateCategoriaById(
		...
		try {
			...
			return ResponseEntity.notFound().build(); // Code 404 -> Categoria no existe		
		} catch(Exception err) {
			return ResponseEntity.internalServerError().build(); // Error 500
		}
	}
	...
	public ResponseEntity<Void> deleteById(@PathVariable Long categoriaId){
		try {
			svc.deleteById(categoriaId);
			return ResponseEntity.noContent().build(); // 204 -> No Content
		} catch(IllegalArgumentException err) {
			return ResponseEntity.notFound().build(); // Code 404 -> Catagoria no existe		
		} catch(Exception err) {
			return ResponseEntity.internalServerError().build(); // Error 500
		}
	}
