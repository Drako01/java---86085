...
public class Clase09Application implements CommandLineRunner{
	...
	public void run(String... args) throws Exception {
		try {
			Categoria cat1 = new Categoria("Programacion Backend");
			Categoria cat2 = new Categoria("Programacion Frontend");
			Categoria cat3 = new Categoria("Data Science");
			Categoria cat4 = new Categoria("Hablidades Blandas");
			...
			Curso curso8 = new Curso("PHP"); // Programacion Backend
			...
			dao.persistirCategoria(cat1); // 1
			dao.persistirCategoria(cat2); // 2
			dao.persistirCategoria(cat3); // 3
			dao.persistirCategoria(cat4); // 4
			...
			List<Long> javaCarrerasIntensivas = new ArrayList<>();
				if(curso9 != null) javaCarrerasIntensivas.add(curso9.getId());
				if(curso10 != null) javaCarrerasIntensivas.add(curso10.getId());
				if(curso2 != null) javaCarrerasIntensivas.add(curso2.getId());
				if(curso3 != null) javaCarrerasIntensivas.add(curso3.getId());
				if(curso4 != null) javaCarrerasIntensivas.add(curso4.getId());
				if(curso1 != null) javaCarrerasIntensivas.add(curso1.getId());
			...
			dao.inscribirAUnAlumnoAUnoOVariosCursos(alumno3.getId(), javaCarrerasIntensivas);
			dao.inscribirAUnAlumnoAUnoOVariosCursos(alumno4.getId(), desarrolloWeb);
			dao.inscribirAUnAlumnoAUnoOVariosCursos(alumno5.getId(), javaCarrerasIntensivas);
