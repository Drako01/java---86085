...
public class CategoriaController {
	...
	public ResponseEntity<?> getCategoriaById(
		...
		try {
			...
			ErrorResponse error = new ErrorResponse(err.getMessage(), "Categoria no encontrada");
	...
	public ResponseEntity<?> updateCategoriaById(
		...
		try {
			...
			ErrorResponse error = new ErrorResponse(err.getMessage(), "Categoria no encontrada");
			return ResponseEntity.status(HttpStatus.NOT_FOUND).body(error);// Code 404 -> Categoria no existe		
		} catch(Exception err) {
			ErrorResponse error = new ErrorResponse(err.getMessage(), "Error Interno del Servidor");
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(error); // Error 500
		}
	...
	public ResponseEntity<?> deleteById(
		...
		try {
			...
			ErrorResponse error = new ErrorResponse(err.getMessage(), "Categoria no encontrada");
			return ResponseEntity.status(HttpStatus.NOT_FOUND).body(error); // Code 404 -> Catagoria no existe		
		} catch(Exception err) {
			ErrorResponse error = new ErrorResponse(err.getMessage(), "Error Interno del Servidor");
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(error); // Error 500
		}
